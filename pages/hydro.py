...
def _build_overlay(s: pd.DataFrame, hydro: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(tickvals=tickvals, ticktext=ticktext, showgrid=False,
                   zeroline=False),
        yaxis=dict(title="monthly z-score", gridcolor=BORDER, zeroline=True,
...
def _build_lag_scatter(s: pd.DataFrame, hydro: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        ...
        margin=dict(l=8, r=8, t=8, b=0),
        height=420,
        xaxis=dict(title="flow z-score", gridcolor=BORDER, zeroline=True,
                   zerolinecolor=BORDER),
        yaxis=dict(title="hydro z-score (next month)", gridcolor=BORDER,
